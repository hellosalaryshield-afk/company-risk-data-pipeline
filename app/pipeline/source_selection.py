...
SEGMENT_SOURCES: dict[str | None, tuple[str, ...]] = {
    INDIA_LISTED: (NEWS_SOURCE, MCA_SOURCE, MARKET_SOURCE, WORKPLACE_SOURCE),
    INDIA_UNLISTED_FUNDED: (NEWS_SOURCE, MCA_SOURCE, WORKPLACE_SOURCE),
    INDIA_UNLISTED_NON_FUNDED: (NEWS_SOURCE, MCA_SOURCE, WORKPLACE_SOURCE),
    FOREIGN_LISTED: (NEWS_SOURCE, MARKET_SOURCE, WORKPLACE_SOURCE),
    FOREIGN_UNLISTED_FUNDED: (NEWS_SOURCE, WORKPLACE_SOURCE),
    FOREIGN_UNLISTED_NON_FUNDED: (NEWS_SOURCE, WORKPLACE_SOURCE),
    ...
    None: (NEWS_SOURCE, WORKPLACE_SOURCE),
}
...
ALL_SOURCES: tuple[str, ...] = (NEWS_SOURCE, MCA_SOURCE, MARKET_SOURCE, WORKPLACE_SOURCE)


def sources_for_segment(segment: str | None, include_metered: bool = True) -> tuple[str, ...]:
    selected = SEGMENT_SOURCES.get(segment, SEGMENT_SOURCES[None])
    if include_metered:
        return selected
    return tuple(name for name in selected if name not in METERED_SOURCES)
